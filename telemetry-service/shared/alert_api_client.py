...
def _trigger_alert_email_sync(
    tank_id: int, quality_log_data: Dict[str, Any], occurred_at_str: str
) -> None:
    """
    ...
    try:
        ...
        logger.error(
            f"Alert API timeout for tank {tank_id} (timeout={config.ALERT_API_TIMEOUT}s)"
        )
...
def trigger_immediate_alert_email(
    tank_id: int, quality_log_data: Dict[str, Any], occurred_at: datetime
) -> bool:
    """
    ...
    try:
        ...
        logger.info(
            f"✓ Triggered alert email API call for tank {tank_id} (background thread)"
        )
        return True  # Return immediately - don't wait for API response

    except Exception as e:
        logger.error(
            f"Error starting alert email thread for tank {tank_id}: {e}", exc_info=True
        )
        return False


def _check_and_create_alerts_sync(
    tank_id: int, branch_id: Optional[int], send_notifications: bool
) -> None:
    """
    Internal function that actually makes the HTTP API call (runs in background thread).
    """
    try:
        ...
        logger.error(
            f"Error calling alert check API for tank {tank_id}: {e}", exc_info=True
        )


def _check_and_create_kpi_alerts_sync(tank_id: int):
    """
    Internal function that actually makes the HTTP API call (runs in background thread).
    """
    try:
        ...
        if response.status_code == 200:
            ...
            alerts = result.get("alerts", [])
            logger.info(
                f"✓ Created/updated {len(alerts)} alert records for tank {tank_id}"
            )
        else:
            logger.warning(
                f"Alert API returned non-200 status: {response.status_code} "
...
def send_refill_detection_to_backend(
    tank_id: int, refill_data: Dict[str, Any], detected_at: datetime
) -> bool:
    """
    ...
    try:
        ...
        logger.info(
            f"✓ Triggered refill detection API call for tank {tank_id} (background thread)"
        )
        return True  # Return immediately - don't wait for API response

    except Exception as e:
        logger.error(
            f"Error starting refill detection thread for tank {tank_id}: {e}",
            exc_info=True,
        )
        return False
...
def _send_refill_detection_sync(api_url: str, payload: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if response.status_code == 200:
            logger.info(
                f"✓ Refill detection sent successfully for tank {payload.get('tank_id')}"
            )
        else:
            logger.warning(
        ...
        logger.error(
            f"Refill detection API timeout for tank {payload.get('tank_id')} (timeout={config.ALERT_API_TIMEOUT}s)"
        )
    except requests.exceptions.ConnectionError as e:
        logger.error(
            f"Refill detection API connection error for tank {payload.get('tank_id')}: {e}"
        )
    except Exception as e:
        logger.error(
            f"Error calling refill detection API for tank {payload.get('tank_id')}: {e}",
            exc_info=True,
        )
...
def check_and_create_alerts(
    ...
    try:
        ...
        logger.info(
            f"✓ Triggered alert check API call for tank {tank_id} (background thread)"
        )
        return []  # Return immediately - don't wait for API response

    except Exception as e:
        logger.error(
            f"Error starting alert check thread for tank {tank_id}: {e}", exc_info=True
        )
        return []


def _check_and_create_refrigerator_kpi_alerts_sync(
    refrigerator_id: int,
    zone_id: Optional[str],
) -> None:
    """
    Internal function that calls the backend endpoint to check and create refrigerator
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            alerts = result.get("alerts", [])
            logger.info(
                f"✓ Created/updated {len(alerts)} alert records for refrigerator_id={refrigerator_id} zone_id={zone_id}"
            )
        else:
            logger.warning(
                f"Alert API returned non-200 status: {response.status_code} "
                f"for refrigerator_id={refrigerator_id}. Response: {response.text[:200]}"
            )

    except Exception as e:
        logger.error(
            f"Error calling alert check API for refrigerator_id={refrigerator_id}: {e}",
            exc_info=True,
        )


def check_and_create_refrigerator_kpi_alerts(
    refrigerator_id: int,
    zone_id: Optional[str] = None,
) -> List[Dict[str, Any]]:
    """
    Check and create KPI deviation alerts for a refrigerator zone via external API
    ...
    try:
        ...
        logger.error(
            f"Error starting refrigerator alert check thread for refrigerator_id={refrigerator_id}: {e}",
            exc_info=True,
        )
        return []
