...
const TrackARC: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'timeline' | 'temperature'>('overview');
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <header className="mb-6">
        ...
        <p className="text-gray-600 mt-1">
          Patient ID: {shipmentData.patientId} | Shipment: {shipmentData.shipmentId}
        </p>
      </header>
      ...
      <div className="bg-blue-600 text-white rounded-lg p-4 mb-6 flex items-center justify-between">
        <div>
          ...
          <h2 className="text-2xl font-bold">{shipmentData.status}</h2>
        </div>
        <div className="text-right">
          <span className="text-sm opacity-80">Estimated Delivery</span>
          <div className="text-lg font-semibold">{shipmentData.estimatedDelivery}</div>
        </div>
      </div>
      ...
      <div className="mt-6 text-center text-sm text-gray-500">
        This is a custom track page variant for ARC Fertility (Hospital ID: 2)
      </div>
    </div>
  );
};
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
      ...
      <div className="space-y-3">
        <DetailRow label="Shipment ID" value={shipmentData.shipmentId} />
        <DetailRow label="Origin" value={shipmentData.origin} />
        <DetailRow label="Destination" value={shipmentData.destination} />
    ...
    <div>
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
        <div className="flex items-center justify-between">
          <div>
            <span className="text-sm text-green-600">Current Temperature</span>
  ...
  return (
    <div className="flex gap-4 pb-6">
      <div className="flex flex-col items-center">
        <div className={`w-4 h-4 rounded-full ${statusColors[event.status]}`} />
        {!isLast && <div className="w-0.5 h-full bg-gray-200 mt-1" />}
      </div>
      <div className="flex-1">
        <div className={`${textColors[event.status]}`}>{event.event}</div>
          ...
          <tbody>
            {temperatureLog.map((entry, index) => (
              <tr key={index} className="border-b border-gray-100">
                <td className="px-4 py-3 text-gray-800">{entry.time}</td>
