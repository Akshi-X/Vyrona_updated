/**
 * @variant SidebarHospital8
 * @hospital ID: 8
 * @route /_sidebar (sentinel key, not a real route)
 * @baseComponent components/Sidebar
 *
 * Refrigerator-only Sidebar for Hospital 8.
 *
 * Hospital 8 only operates the refrigerator module (no cryotanks,
 * incubators, embryo console, database, or control tower). This
 * variant keeps the default Sidebar's look/behavior (mobile drawer,
 * onboarding-mode path prefixing, profile row, logout flow) but trims
 * the nav items down to what's relevant for a refrigerator-only site.
 *
 * Registry key: SidebarHospital8 (auto from hospital-8/Sidebar.tsx)
 */
...
const DASHBOARD_CHILD_PATHS = ["/dashboard", "/refrigerator-tracking"];

const isDashboardRoute = (pathname: string) =>
    DASHBOARD_CHILD_PATHS.some((p) => pathname === p || pathname.startsWith(p + "/"));

const isAlertConfigRoute = (pathname: string) =>
    pathname === "/alert-setting" || pathname.startsWith("/alert-setting/");

// Refrigerator-only nav: no Database, Control Tower, Refill log, or
// cryotank/incubator/embryo dashboard children.
...
const SidebarHospital8: React.FC<SidebarProps> = ({ onLogout }) => {
    ...
    const resolvePath = (path: string) => (isOnboarding ? `/onboarding${path}` : path);
        ...
        <>
            ...
            >
                ...
                <header className="flex items-center gap-[7px] px-6 pt-0 pb-2 md:pb-6 flex-shrink-0 relative z-10">
                    <img src={MyGrapeLogo} alt="myGrape logo" className="w-[110px] h-[75px] md:w-[135px] md:h-[90px]" />
                </header>
                ...
                <nav className="flex flex-col gap-[18px] px-6 pb-2 flex-1 overflow-y-auto relative z-10 scrollbar-none">
                    {navigationItems.map((item, index) => {
                                ...
                                <div key={index} className="flex flex-col gap-0.5">
                                    <button
                                        ...
                                        className={`h-auto w-full justify-between gap-4 px-3 py-[7px] rounded-[10px] flex items-center transition-colors ${
                                            active ? "bg-white/20" : isOpen ? "bg-white/10" : "bg-transparent hover:bg-white/10"
                                        }`}
                                    >
                        ...
                        const needsInvert = label === "Alert Config" && !isActive;
...
export default SidebarHospital8;
