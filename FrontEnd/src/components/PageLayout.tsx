...
const PageLayout = ({ title, description, icon, iconAlt, lucideIcon, actions, children, hideHeaderOnDesktop = false, patternBackground = false, hideTourButton = false, titleBadge, titleSuffix }: PageLayoutProps) => {
    return (
        <main id="onboarding-page-layout" className={`flex flex-col h-dvh overflow-hidden page-enter ${patternBackground ? "relative isolate" : ""}`}>
            ...
            <div className={`${hideHeaderOnDesktop ? "md:hidden" : "md:px-6 md:pt-10 md:pb-6"} flex-shrink-0 px-4`}>
                <PageHeader title={title} description={description} icon={icon} iconAlt={iconAlt} lucideIcon={lucideIcon} actions={actions} hideTourButton={hideTourButton} titleBadge={titleBadge} titleSuffix={titleSuffix} />
            </div>
