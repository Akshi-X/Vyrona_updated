...
const CriticalAlertsModal: React.FC<CriticalAlertsModalProps> = ({
  ...
  const groupedAlerts = visibleAlerts.reduce<Record<string, CriticalAlert[]>>((acc, alert) => {
  ...
  const groupedByDateAndKpi = Object.entries(groupedAlerts).reduce<
    Record<string, { groupKey: string; tankGroupId: string; kpiConfigId: string; alerts: CriticalAlert[] }[]>
  >((acc, [dateLabel, dateAlerts]) => {
    const groupedMap = dateAlerts.reduce<Record<string, CriticalAlert[]>>((inner, alert) => {
    ...
    const groups = Object.entries(groupedMap).map(([compositeGroupId, kpiAlerts]) => {
      const sortedAlerts = [...kpiAlerts].sort(
        (a, b) => parseTimestampValue(b.timestamp) - parseTimestampValue(a.timestamp)
      );
      const [tankGroupId = '', kpiConfigId = ''] = compositeGroupId.split('__');
      return {
    ...
    groups.sort(
      (a, b) => parseTimestampValue(b.alerts[0]?.timestamp || '') - parseTimestampValue(a.alerts[0]?.timestamp || '')
    );
  ...
  const toggleGroupExpansion = (groupKey: string) => {
        ...
        {isFilterPanelOpen && (
          <div className="absolute right-0 top-full mt-2 z-[9999] bg-white border border-[#e7c6ec] rounded-xl shadow-xl p-4 min-w-[260px]">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-semibold text-[#6b1176]">Filters</span>
              {activeFilterCount > 0 && (
                <button
                  onClick={() => {
                    setPriorityFilter('all');
    ...
    return (
      ...
      >
        ...
        {Object.entries(groupedByDateAndKpi).map(([dateLabel, dateGroups]) => (
              ...
              {dateGroups.map((group) => {
                ...
                return (
                  <div
                    ...
                    onClick={hiddenCount > 0 ? () => toggleGroupExpansion(group.groupKey) : undefined}
                  >
                        ...
                        <div className="min-w-0">
                          <div className="mt-1 flex flex-wrap items-center gap-2">
                            ...
                            <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                              latestAlert.severity === 'Critical' || latestAlert.severity === 'High' ? 'bg-red-50 text-red-700' :
                              latestAlert.severity === 'Low' ? 'bg-orange-50 text-orange-700' :
                              'bg-orange-50 text-orange-700'
                            }`}>
                              {latestAlert.severity}
                            </span>
                            <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                              latestAlert.status === 'Active' ? 'bg-green-50 text-green-700' :
                              latestAlert.status === 'Acknowledged' ? 'bg-gray-100 text-gray-600' :
                              'bg-gray-100 text-gray-600'
                            }`}>
                              {latestAlert.status}
                            </span>
                          </div>
                          <div className="mt-1 text-sm text-[#333333]">{latestAlert.message}</div>
                          {showPatientId && (
                            <>
                              <div className="mt-2 flex flex-wrap items-center gap-x-2 text-sm text-[#333333]">
                                <span className="font-semibold text-[#6b1176]">{patientIdLabel}:</span>{' '}
                                <span className="font-semibold text-[#1f2937] tracking-wide">{getPatientDisplay(latestAlert)}</span>
                              </div>
                              <div className="mt-1 text-xs text-gray-500">{formatAlertTime(latestAlert.timestamp)}</div>
                            </>
                          )}
                          {!showPatientId && (
                            <div className="mt-2 text-xs text-gray-500">{formatAlertTime(latestAlert.timestamp)}</div>
                          )}
                        </div>
                      </div>
                    ...
                    {isExpanded && hiddenCount > 0 && (
                      <div className="mt-3 border-t border-gray-200/70 pt-3 space-y-3">
                        {olderAlerts.map((alert) => (
                          <div key={alert.id} className="flex items-center justify-between gap-3 pl-12">
                            <div className="min-w-0 text-left">
                              <div className="text-sm text-[#333333]">{alert.message}</div>
                              <div className="mt-1 text-xs text-gray-500">{formatAlertTime(alert.timestamp)}</div>
                            </div>
                            {onAcknowledge && (
                              <>
                                {alert.status === 'Active' ? (
