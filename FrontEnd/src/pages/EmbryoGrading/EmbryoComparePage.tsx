      ...
      <div className="flex gap-4 flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center gap-1 mt-0.5">
                <p className="text-xs text-gray-400">Embryos ranked by AI Score</p>
          ...
          <div className="flex flex-col gap-2.5">
            {LEADERBOARD_EMBRYOS.map((emb) => {
              ...
              return (
                <div
                  key={emb.id}
                  className="relative flex items-center gap-3 px-3.5 py-3.5 rounded-2xl cursor-pointer transition-all bg-white"
                  style={{
                ...
                >
                  ...
                  <div className="flex flex-col items-center gap-2 shrink-0 w-8 pl-1.5">
                    {rankMedal ? (
                      <div
                      ...
                      <div className="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold text-gray-400 border border-gray-200 bg-gray-50">
                        {emb.rank}
                      </div>
                    )}
                    <div
                      className="w-[15px] h-[15px] rounded-[4px] flex items-center justify-center transition-all"
                  ...
                  {/* Thumbnail */}
                  <div
                    className="w-[58px] h-[58px] rounded-xl overflow-hidden shrink-0"
                    style={{
                      border: isSelected && color
                        ? `2px solid ${color.border.replace('border-[','').replace(']','')}`
                        : '1.5px solid #E5E0EE',
                    }}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline gap-2 mb-2">
                      <span className="text-sm font-bold text-gray-900 leading-none">{emb.id}</span>
                      <span className="text-[11px] text-gray-400 leading-none">{emb.time}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-[11px] font-bold text-[#7c3aed] bg-[#F3E8FF] px-2 py-0.5 rounded-md border border-[#ddd0f5]">{emb.grade}</span>
                      <span className="text-[10px] text-gray-400 font-medium">{emb.quality}</span>
                    </div>
                  </div>

                  {/* AI Score */}
                  <div className="shrink-0 flex flex-col items-center rounded-xl bg-[#FAF5FF] px-3 py-2.5 min-w-[56px] border border-[#EDE0FA]">
                    <span className="text-[8px] font-bold text-[#c084fc] uppercase tracking-widest leading-none mb-1">AI Score</span>
                    <span className="text-[22px] font-black leading-none text-gray-800">{emb.aiScore}</span>
                  </div>
                </div>
              );
            })}
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-3 overflow-auto">
          <div className="flex items-center justify-between shrink-0">
            <div>
              <h3 className="text-base font-bold text-gray-900">Compare Embryos</h3>
              <p className="text-xs text-gray-400 mt-0.5">Select up to 4 embryos to compare</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-[#6b1176] bg-[#F7ECFF] px-2.5 py-1 rounded-full border border-[#c084fc]/40">
          ...
          {selectedEmbryoIds.length === 0 ? (
            <div className="flex flex-col items-center justify-center flex-1 py-16 text-center">
              <div className="w-14 h-14 rounded-full bg-[#F7ECFF] flex items-center justify-center mb-3 text-[#6b1176]">
            ...
            <div className="overflow-x-auto">
              <div style={{ minWidth: `${160 + selectedEmbryoIds.length * 200}px` }}>

                {/* Embryo columns — offset by 160px to align with metrics label */}
                <div
                  className="grid gap-3 mb-3"
                  style={{ gridTemplateColumns: `160px repeat(${selectedEmbryoIds.length}, minmax(180px, 260px))` }}
                >
                  ...
                  {selectedEmbryoIds.map((eid, idx) => {
                    ...
                    return (
                      <div key={eid} className="rounded-2xl overflow-hidden flex flex-col bg-white" style={{ border: `1px solid ${color.border.replace('border-[','').replace(']','')}`, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>

                        {/* Header */}
                        <div className="flex items-center gap-2 px-3 py-2.5 border-b" style={{ borderColor: color.border.replace('border-[','').replace(']','') + '40', background: color.badgeBg.replace('bg-[','').replace(']','') + '12' }}>
                          <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black text-white shrink-0" style={{ background: color.badgeBg.replace('bg-[','').replace(']','') }}>{idx + 1}</span>
                          <span className="text-sm font-bold text-gray-800 flex-1 truncate">{emb.id}</span>
                          <span className="text-[11px] text-gray-400 shrink-0">{emb.time}</span>
                          <button
                            type="button"
                        ...
                        <div className="relative p-2.5">
                          ...
                          <div className="absolute top-4 right-4 w-5 h-5 rounded-md flex items-center justify-center shadow-sm" style={{ background: color.badgeBg.replace('bg-[','').replace(']','') }}>
                            <svg width="8" height="8" viewBox="0 0 10 10" fill="none"><path d="M2 5l2.5 2.5 4-4" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                          </div>
                        </div>
                        ...
                        <div className="px-3 pb-3">
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">AI Score</span>
                            <span className="text-base font-black text-gray-800">{emb.aiScore} <span className="text-xs font-medium text-gray-400">/ 10</span></span>
                          </div>
                        ...
                        <div className="flex gap-1.5 px-3 pb-3.5">
                          {[{ label: 'ICM', value: emb.icm }, { label: 'TE', value: emb.te }, { label: 'Expansion', value: emb.expansion }].map(chip => (
                            <div key={chip.label} className="flex-1 rounded-xl bg-gray-50 border border-gray-100 px-1 py-2 text-center">
                              <div className="text-[9px] text-gray-400 font-semibold uppercase tracking-wide leading-none mb-1">{chip.label}</div>
                              <div className="text-sm font-black text-gray-700">{chip.value}</div>
                            </div>
                          ))}
                        </div>
                      </div>
                ...
                {/* Key Metrics — same column template, label fills the first 160px slot */}
                <div className="rounded-xl border border-[#E7E1E1] overflow-hidden">
                  <div className="px-4 py-3 border-b border-[#E7E1E1] bg-[#FDFAFF]">
                  ...
                  <div className="divide-y divide-[#F0EBF4]">
                    {([
                      ...
                      <div
                        ...
                        style={{ gridTemplateColumns: `160px repeat(${selectedEmbryoIds.length}, minmax(180px, 260px))` }}
                      >
                        <div className="px-4 py-2.5 text-xs font-semibold text-gray-500 flex items-center border-r border-[#F0EBF4]">{row.metric}</div>
                        {selectedEmbryoIds.map((eid, idx) => {
                          const emb = LEADERBOARD_EMBRYOS.find(e => e.id === eid)!;
                          const color = SLOT_COLORS[idx];
                          return (
                            <div key={eid} className="px-3 py-2.5 flex items-center gap-2 border-r border-[#F0EBF4] last:border-r-0">
                              {row.bars && (
                                <div className="flex gap-0.5 shrink-0">
                                  {[0, 1, 2, 3].map(i => (
                                    <div key={i} className={`w-4 h-2 rounded-sm ${i < row.bars!(emb) ? color.barCol : 'bg-gray-100'}`} />
                                  ))}
                                </div>
                              )}
                              <span className="text-xs text-gray-700">{row.getValue(emb)}</span>
                            </div>
                          );
                        })}
                      </div>
