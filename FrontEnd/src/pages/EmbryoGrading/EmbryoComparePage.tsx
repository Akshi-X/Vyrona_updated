...
import { useEmbryoGrades, gradeCls, scoreBarGradient, scorePillStyle, criticalStyle } from './useEmbryoGrades';
...
export default function EmbryoComparePage() {
  ...
  const { embryos, loading } = useEmbryoGrades(cycle);
  ...
  useEffect(() => {
    setSelectedOocyteId(prev => (prev && embryos.some(e => e.id === prev)) ? prev : (embryos[0]?.id ?? ''));
  }, [embryos]);
