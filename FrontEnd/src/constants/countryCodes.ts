...
/** All countries with ITU dial codes, sorted by name. */
...
/** Longest-dial-prefix match for a full E.164 number like "+919876543210". */
export const findCountryByPhone = (phone: string): Country | undefined => {
    const normalized = `+${phone.replace(/\D/g, "")}`;
    let best: Country | undefined;
    for (const c of COUNTRIES) {
        if (normalized.startsWith(c.dial) && (!best || c.dial.length > best.dial.length)) {
            best = c;
        }
    }
    return best;
