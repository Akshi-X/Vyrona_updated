...
def get_active_routes(
    ...
    carriers: Optional[List[str]] = Query(None, description="Filter by carrier name (single value)"),
    regions: Optional[List[str]] = Query(None, description="Filter by a single region. Matches if either source or destination is in the specified region, e.g., 'Europe', 'North America', 'Asia'"),
    db: Session = Depends(get_db)
):
    """
