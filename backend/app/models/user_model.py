...
class User(Base):
    ...
    email = sqlalchemy.Column(
        sqlalchemy.String, unique=True, nullable=False, index=True
    )
    ...
    branch_id = sqlalchemy.Column(
        Integer, ForeignKey("hospital_branches.branch_id"), nullable=True, index=True
    )
    department = sqlalchemy.Column(
        sqlalchemy.String(100), nullable=True, index=True
    )  # CGT, IVF, Oncology, etc.
    hospital_id = sqlalchemy.Column(
    ...
    approved_status = sqlalchemy.Column(
        SQLEnum(
            ApprovalStatus,
            values_callable=lambda obj: [e.value for e in obj],
    ...
    lock_expiry = sqlalchemy.Column(
        sqlalchemy.DateTime(timezone=True), nullable=True
    )  # When lock expires (30 min)
    login_attempts = sqlalchemy.Column(
        sqlalchemy.Integer, default=0
    )  # Failed login counter

    # Password Security
    last_password_changed = sqlalchemy.Column(
        sqlalchemy.DateTime, default=lambda: datetime.now(timezone.utc)
    )
    last_password_reset_request = sqlalchemy.Column(
        sqlalchemy.DateTime, nullable=True
    )  # Audit: last reset request
    password_reset_count = sqlalchemy.Column(
        sqlalchemy.Integer, default=0
    )  # Audit: total reset attempts

    # Session Management
    session_timeout = sqlalchemy.Column(
        sqlalchemy.Integer, default=30
    )  # Minutes (custom per user)
    last_login = sqlalchemy.Column(
        sqlalchemy.DateTime, nullable=True
    )  # Last successful login

    # Onboarding
    onboarding_completed = sqlalchemy.Column(
    ...
    invite_token_expires_at = sqlalchemy.Column(
        sqlalchemy.DateTime(timezone=True), nullable=True
    )
    ...
    pharma_company = relationship(
        "Pharma", foreign_keys="[User.pharma_id]", back_populates="users"
    )
