...
from ..constants.app_constants import (
    REMEMBER_ME_SESSION_DURATION_MINUTES,
    NO_REMEMBER_ME_SESSION_DURATION_MINUTES,
)
...
def send_otp_to_user(
    db: Session, user_id: str, email: str, remember_me: bool = False
) -> OTP:
    """
...
def verify_otp(db: Session, user_id: str, otp_code: str) -> bool:
    ...
    In FIXED_OTP_MODE, only the configured fixed code is accepted.
    In production mode, the OTP must match the most recent unused record.
    """
    try:
        # Find the most recent unused OTP for the user
        otp_record = (
        ...
        if settings.FIXED_OTP_MODE:
            # In fixed mode, accept only the fixed OTP code.
            if otp_code != settings.FIXED_OTP_CODE:
                otp_record.attempts += 1
                db.commit()
                return False
        else:
            # In production, verify exact OTP match.
...
def verify_otp_and_create_token(user_id: str, otp: str, db: Session) -> dict:
    ...
    try:
        ...
        if otp_record and otp_record.remember_me:
            ...
            remember_me = True
            logger.info(
                f"Remember Me enabled: Session expires in {session_duration} minutes (9 hours)"
            )
        else:
            session_duration = NO_REMEMBER_ME_SESSION_DURATION_MINUTES  # 1 hour
            remember_me = False
            logger.info(
                f"Remember Me disabled: Session expires in {session_duration} minutes (1 hour)"
            )

        # Determine if hospital user based on department
        is_hospital_user = (
            is_hospital_department(user.department) if user.department else False
        )
        audit_log_disabled = is_audit_log_disabled_for_user(user)
        ...
        if is_hospital_user:
            ...
            token_data["department"] = (
                user.department
            )  # Include department (CGT) for pharma users
            token_data["type"] = "pharma_user"

        # Business Logic: Create JWT token with remember_me flag and type-specific fields
        logger.debug(
            f"Creating JWT token with session duration: {session_duration} minutes"
        )
        ...
        if is_hospital_user:
            ...
            hospital = (
                db.query(Hospital)
                .filter(Hospital.hospital_id == branch.hospital_id)
                .first()
                if branch
                else None
            )
