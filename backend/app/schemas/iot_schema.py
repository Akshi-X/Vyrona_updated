...
class WebhookHeaderSchema(BaseModel):
    """Webhook header schema - matches Tive API structure"""
...
class WebhookCreateRequest(BaseModel):
    """Request schema for creating webhook - matches Tive API Swagger structure exactly
    ...
    """
    name: str = Field(..., description="Webhook name (required)")
    url: str = Field(..., description="Webhook URL where Tive will send events (required)")
    ...
    template: Optional[str] = Field(None, description="Webhook template")
    headers: Optional[List[WebhookHeaderSchema]] = Field(None, description="Custom headers to include in webhook requests")
    accountIds: Optional[List[int]] = Field(None, description="List of account IDs this webhook applies to")
    httpMethod: Optional[str] = Field("POST", description="HTTP method (default: POST)")
    contentType: Optional[str] = Field("application/json", description="Content type (default: application/json)")
    enabled: Optional[bool] = Field(True, description="Whether the webhook is enabled (default: true)")
    applyToAllTrackers: Optional[bool] = Field(False, description="Apply to all trackers (default: false)")
    applyToAllShipments: Optional[bool] = Field(False, description="Apply to all shipments (default: false)")
    applyToAllAlertPresets: Optional[bool] = Field(False, description="Apply to all alert presets (default: false)")
