...
class ActiveTankItem(BaseModel):
    ...
    status: CanisterStatus = Field(..., description="Tank status (safe, risk, critical, offline)")
...
class BranchRefrigerators(BaseModel):
    branch_id: int
    branch_name: str
    refrigerators: List[ActiveRefrigeratorItem]

    class Config:
        from_attributes = True


class ActiveRefrigeratorsResponse(BaseModel):
    branches: List[BranchRefrigerators]
    total: int
