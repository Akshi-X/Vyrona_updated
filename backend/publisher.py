...
def find_ivf_shipment_by_identifiers(db, shipment_id: Optional[str] = None, 
    ...
    IVF shipments move cryolocks (not canisters), so we get tank_id from patient_crylock_info.
    
    Returns dict with: shipment_id, patient_crylock_info_id, tank_id, device_id, canister_number, or None if not found.
    """
    try:
        # IVF shipments reference patient_crylock_info, which has tank_id
        # canister_number is stored in patient_crylock_info as extracted component
        # tank_code is stored in tanks table
        query = text("""
            SELECT 
                ivf.shipment_id,
                ivf.iot_shipment_id,
                ivf.patient_crylock_info_id,
                ivf.device_id,
            ...
            LEFT JOIN patient_crylock_info pci ON ivf.patient_crylock_info_id = pci.id
            LEFT JOIN tanks t ON pci.tank_id = t.tank_id
            WHERE 
        ...
        if row:
            return {
                ...
                "patient_crylock_info_id": row[2],
                "device_id": row[3],
                "source_branch_id": row[4],
                "destination_branch_id": row[5],
                "tank_id": row[6],  # Get tank_id from patient_crylock_info
                "canister_number": row[7],  # Get canister_number from patient_crylock_info (extracted component)
                "tank_code": row[8]  # Get tank_code from tanks table
            }
        return None
    except Exception as e:
...
def find_tank_by_tive_device_id(db, device_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
...
def insert_ivf_telemetry_data(tank_id: int, data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        # (Tank model stores tracker id as `tive_device_id`)
        webhook_device_id = data.get("device_id")
        if not webhook_device_id:
            try:
                tank_device_row = db.execute(
                ...
                # Best-effort only; don't block telemetry insert
                pass
        ...
        logger.info(f"✓ Inserted IVF telemetry data for tank {tank_id} (id={ivf_telemetry_data_id})")
        print(f"  → Inserted into database table 'ivf_telemetry_data' (id={ivf_telemetry_data_id}, tank_id: {tank_id})", flush=True)
...
def insert_ivf_quality_log(db, ivf_telemetry_data_id: int, tank_id: int, 
    ...
    try:
        ...
        # KPI statuses: "Good" = no loss, "Warning" or "Critical" = loss
        # Note: Humidity is not monitored for IVF
        is_temp_internal_loss = kpi_statuses.get("temp_internal", "Good") in ["Warning", "Critical"]
        is_temp_external_loss = kpi_statuses.get("temp_external", "Good") in ["Warning", "Critical"]
        is_humidity_loss = False  # Humidity not monitored for IVF
        is_shock_loss = kpi_statuses.get("shock", "Good") in ["Warning", "Critical"]
        
        # Build insert query - IVF KPIs: Temp Internal, Temp External, Shock (humidity not monitored)
        # Database model has separate columns for temperature_internal, temperature_external, shock
        insert_query = text("""
            INSERT INTO ivf_quality_log 
            (telemetry_data_id, tank_id, device_id, temperature_internal, temperature_external, humidity, shock, 
             quality_loss, is_temp_internal_loss, is_temp_external_loss, is_humidity_loss, is_shock_loss, reading_timestamp, created_at)
            VALUES (:telemetry_data_id, :tank_id, :device_id, :temperature_internal, :temperature_external, :humidity, :shock,
                    :quality_loss, :is_temp_internal_loss, :is_temp_external_loss, :is_humidity_loss, :is_shock_loss, :reading_timestamp, :created_at)
        """)
        
        # Prefer webhook device_id; if missing, fallback to tank's mapped tracker id (`tive_device_id`)
        webhook_device_id = data.get("device_id")
        if not webhook_device_id:
            try:
                tank_device_row = db.execute(
        ...
        db.execute(insert_query, {
            ...
            "tank_id": tank_id,
            "device_id": webhook_device_id,
            "temperature_internal": data.get("temp_internal"),  # Temperature Internal
            "temperature_external": data.get("temp_external"),  # Temperature External
            "humidity": data.get("humidity"),  # Humidity
            "shock": data.get("shock"),  # Shock
        ...
        logger.info(f"✓ Inserted into ivf_quality_log table (telemetry_data_id={ivf_telemetry_data_id}, tank_id={tank_id}, quality_loss={quality_loss})")
        print(f"  → Inserted into ivf_quality_log table (telemetry_data_id={ivf_telemetry_data_id}, quality_loss={quality_loss})", flush=True)
        
        # Trigger immediate alert email notification (IVF only)
        # This sends emails to branch users if occurrence < 3, or managers if >= 3
        try:
            if CRITICAL_ALERT_SERVICE_AVAILABLE and CriticalAlertService is not None:
                # Prepare quality log data for email trigger
                quality_log_data = {
                    "is_temp_internal_loss": is_temp_internal_loss,
                    "is_temp_external_loss": is_temp_external_loss,
                    "is_shock_loss": is_shock_loss,
                ...
                # Create alert service instance and send immediate email
                alert_service = CriticalAlertService(db)
                alert_service.send_immediate_alert_email(
                    tank_id=tank_id,
                    quality_log_data=quality_log_data,
                    occurred_at=created_at
                )
                logger.info(f"✓ Triggered immediate alert email check for tank {tank_id} (check logs above for email send status)")
                print(f"  ✓ Triggered immediate alert email check for tank {tank_id}", flush=True)
            else:
                logger.warning("CriticalAlertService not available - skipping immediate alert email")
                print(f"  ⚠ Warning: CriticalAlertService not available - emails will not be sent", flush=True)
        except Exception as e:
            logger.error(f"Error triggering immediate alert email for tank {tank_id}: {e}", exc_info=True)
            # Don't fail the insert if email trigger fails
            print(f"  ⚠ Warning: Failed to trigger immediate alert email: {e}", flush=True)
...
def calculate_quality_status_from_kpis(kpi_scores: Dict[str, int]) -> tuple[str, float]:
    ...
    IVF-specific rules:
    - If Internal Temp (temp_internal) = Red → Force Overall = Red
    - If Shock = Red → Force Overall = Red
    - Otherwise use scoring model
    ...
    # IVF-specific: Check if temp_internal or shock is Red - force Red status
    temp_internal_red = kpi_scores.get("temp_internal", 0) == 2
    shock_red = kpi_scores.get("shock", 0) == 2
    ...
    # IVF: Force Red if Internal Temp = Red OR Shock = Red
    if temp_internal_red or shock_red or has_red:
        # Force Red if critical KPIs are Red (IVF safety rule)
...
def check_ivf_threshold_magnitude(parameter_name: str, value: float) -> Dict[str, Any]:
    ...
    - For other KPIs (humidity): Acceptable band: ±5 from target
      - within ±5 = Green (acceptable)
      - deviation > ±5 = triggers alert
    ...
    if parameter_name == "temp_internal":
        # Nominal setpoint: -196°C, ±5°C acceptable band
        # -201°C to -191°C = Yellow (Minor cryogenic fluctuation) or Green (Stable cryogenic storage)
        # Outside this range = Red (Critical cryogenic breach)
        ...
        if acceptable_min <= value <= acceptable_max:
            ...
            if -197.0 <= value <= -195.0:
                # Very close to -196°C (±1°C) = Green (Stable cryogenic storage)
                magnitude_status = "Green"
                is_violation = False
                deviation = 0.0
                triggers_alert = False
            else:
                # Within acceptable range but not very close = Yellow (Minor cryogenic fluctuation)
            ...
            is_violation = True
            triggers_alert = True  # Critical alert for cryogenic breach
    ...
    if parameter_name == "shock":
        ...
        if value <= threshold:
            ...
            magnitude_status = "Red"
            is_violation = True
            triggers_alert = True  # Immediate Critical Alert
    ...
    if parameter_name == "temp_external":
        ...
        if operating_min <= value <= operating_max:
            # Within operating range (-20°C to 60°C) - Green
            magnitude_status = "Green"
            is_violation = False
            deviation = 0.0
            triggers_alert = False
            ...
            else:  # value > operating_max
                deviation = value - operating_max
            
            # ≤5°C outside = Yellow, >5°C outside = Red
            if deviation <= 5.0:
                magnitude_status = "Yellow"
            else:
                magnitude_status = "Red"
    ...
    # Standard logic for other KPIs (humidity): ±5 acceptable band
    ...
    # Logic: within ±5 = Green, beyond ±5 = triggers alert
    if deviation == 0.0:
        ...
        # Beyond ±5 from target = triggers alert
        magnitude_status = "Red"
        is_violation = True
        triggers_alert = True
...
def get_ivf_violation_frequency(tank_id: int, parameter_name: str, window_minutes: int = 60) -> Dict[str, Any]:
    ...
    try:
        ...
        # Determine which violation flag to check (IVF KPIs: Temp Internal, Temp External, Shock - humidity not monitored)
        violation_flag_map = {
...
def transform_webhook_to_ivf_quality_data(webhook_response: Dict[str, Any], ivf_shipment_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # IVF KPIs: Temperature Internal, Temperature External, Shock (humidity not monitored)
    # IVF shipments move cryolocks (not canisters), get tank_id from patient_crylock_info
    ...
    # Special cases:
    # - temp_internal: Cryogenic storage at -196°C, ±5°C acceptable band (-201°C to -191°C)
    # - temp_external: Operating range -20°C to 60°C
    # - shock: Threshold 1G (≤1G = Green, >1G = Red)
    thresholds = {}
    for param_name in ivf_kpi_names:
        ...
        if param_name == "temp_internal":
            # Internal temp: Cryogenic storage at -196°C, ±5°C acceptable band
            thresholds[param_name] = {
                "target": target,  # -196°C (nominal setpoint)
                "min": -201.0,  # -196°C - 5°C
                "max": -191.0,  # -196°C + 5°C
                "unit": "°C"
            }
        elif param_name == "temp_external":
            # External temp: Operating range -20°C to 60°C
            thresholds[param_name] = {
    ...
    for param_name in ivf_kpi_names:
        ...
        # tank_id is already extracted from ivf_shipment_info (from patient_crylock_info relationship)
        if tank_id:
            ...
            # If no tank_id available, return default values
        ...
        if param_name in magnitude_results and parameters.get(param_name) is not None:
            ...
            if deviation > 0:
                ...
                if "temp" in param_name:
                    weight = 0.45  # Both temps share weight (increased since humidity removed)
                elif param_name == "shock":
                    weight = 0.20  # Increased since humidity removed
                else:
                    weight = 0.1
    ...
    data = {
        "patient_crylock_info_id": ivf_shipment_info.get("patient_crylock_info_id"),  # Cryolock being shipped
        "tank_id": ivf_shipment_info.get("tank_id"),  # Tank ID from patient_crylock_info
        "tank_code": ivf_shipment_info.get("tank_code"),  # Tank code (e.g., "T1", "T10") for frontend matching
        "canister_number": ivf_shipment_info.get("canister_number"),  # Extracted component from crylock_number (for display/backward compatibility)
        "shipment_id": shipment_id,
...
def process_webhook_data(webhook_response: Dict[str, Any]):
    ...
    try:
        ...
        try:
            ...
            # Check 1: Tank-level monitoring (always check if device_id exists)
            tank_info = None
            if device_id:
                tank_info = find_tank_by_tive_device_id(db, device_id)
                if tank_info:
                    logger.info(f"✓ Found IVF tank (monitoring): tank_id={tank_info.get('tank_id')}, device_id={device_id}")
                    print(f"✓ Found IVF tank (monitoring): tank_id={tank_info.get('tank_id')}, device_id={device_id}", flush=True)
            
            # Check 2: Shipment monitoring (ALSO check - don't stop after tank)
            if shipment_id or device_id:
                ...
                if ivf_shipment_info:
                    logger.info(f"✓ Found IVF shipment: shipment_id={ivf_shipment_info.get('shipment_id')}, tank_id={ivf_shipment_info.get('tank_id') if 'tank_id' in ivf_shipment_info else 'N/A'}")
                    print(f"✓ Found IVF shipment: shipment_id={ivf_shipment_info.get('shipment_id')}", flush=True)
        except Exception as e:
        ...
        # ========================================================================
        # STEP 2: Process IVF tank-level monitoring (if found)
        # ========================================================================
        if tank_info:
            try:
                logger.info("Processing IVF tank-level monitoring webhook...")
                print(f"Processing IVF tank-level monitoring webhook...", flush=True)
                
                # Convert tank_info to same format as ivf_shipment_info for processing
                tank_ivf_info = {
                    "shipment_id": None,
                    ...
                    "device_id": device_id,
                    "cryolock_id": None,
                    "source_branch_id": tank_info.get("branch_id"),
                ...
                try:
                    ...
                    logger.info(f"Published to Redis channel 'ivf_quality_channel' for tank {tank_id} (tank monitoring)")
                    print(f"  → Published to Redis channel 'ivf_quality_channel' (tank monitoring)", flush=True)
                except Exception as e:
                    logger.error(f"Error publishing to IVF Redis channel for tank {tank_id}: {e}")
                    print(f"  ✗ Error publishing to IVF Redis channel: {e}", flush=True)
                
                # Store in tank-specific history list
                try:
                    ...
                    logger.info(f"Stored in Redis list '{history_key}' for tank {tank_id} (tank monitoring)")
                    print(f"  → Stored in Redis list '{history_key}' (tank monitoring)", flush=True)
                except Exception as e:
                    logger.error(f"Error storing in IVF Redis list for tank {tank_id}: {e}")
                    print(f"  ✗ Error storing in IVF Redis list: {e}", flush=True)
                
                # Store tank ID in set
                try:
                    r.sadd('ivf_tanks', str(tank_id))
                    logger.debug(f"Added tank {tank_id} to Redis set 'ivf_tanks'")
                except Exception as e:
                    logger.error(f"Error adding tank {tank_id} to Redis set: {e}")
                
                # Insert into IVF database tables
                try:
                    insert_ivf_telemetry_data(tank_id, ivf_data)
                    logger.info(f"✓ Inserted IVF telemetry data for tank {tank_id} (tank monitoring)")
                    print(f"✓ Inserted IVF telemetry data for tank {tank_id} (tank monitoring)", flush=True)
                except Exception as e:
                    logger.error(f"Error inserting IVF telemetry data for tank {tank_id}: {e}", exc_info=True)
                    print(f"  ✗ Error inserting IVF telemetry data: {e}", flush=True)
                
                logger.info(f"✓ Successfully processed IVF tank-level monitoring for tank {tank_id}")
                print(f"✓ Successfully processed IVF tank-level monitoring for tank {tank_id} at {ivf_data['timestamp']}", flush=True)
                ivf_processed = True
                
            except Exception as e:
                logger.error(f"Error processing IVF tank-level monitoring: {e}", exc_info=True)
                print(f"✗ Error processing IVF tank-level monitoring: {e}", flush=True)
                import traceback
                traceback.print_exc()
        ...
        if ivf_shipment_info:
            try:
                ...
                # Get tank_id from shipment info (IVF shipments reference patient_crylock_info which has tank_id)
                # tank_id is already included in ivf_shipment_info from find_ivf_shipment_by_identifiers
                tank_id = ivf_shipment_info.get("tank_id")
                
                # Fallback: If tank_id not in shipment info, try to get from device_id
                if not tank_id and device_id:
                    db_shipment = PublisherSessionLocal()
                    try:
                        tank_info_from_device = find_tank_by_tive_device_id(db_shipment, device_id)
                        if tank_info_from_device:
                            tank_id = tank_info_from_device.get("tank_id")
                    finally:
                        db_shipment.close()
                
                if not tank_id:
                    logger.warning(f"Could not resolve tank_id for IVF shipment {ivf_shipment_info.get('shipment_id')} - skipping")
                    print(f"  ⚠ Could not resolve tank_id for shipment - skipping", flush=True)
                    raise ValueError("Could not resolve tank_id for IVF shipment")
                ...
                try:
                    r.publish('ivf_quality_channel', json.dumps(ivf_data))
                    logger.info(f"Published to Redis channel 'ivf_quality_channel' for tank {tank_id} (shipment monitoring)")
                ...
                try:
                    ...
                    r.lpush(history_key, json.dumps(ivf_data))
                    r.ltrim(history_key, 0, 9)  # Keep last 10 messages
                    logger.info(f"Stored in Redis list '{history_key}' for tank {tank_id} (shipment monitoring)")
                ...
                try:
                    insert_ivf_telemetry_data(tank_id, ivf_data)
                    logger.info(f"✓ Inserted IVF telemetry data for tank {tank_id} (shipment monitoring)")
                    print(f"✓ Inserted IVF telemetry data for tank {tank_id} (shipment monitoring)", flush=True)
                except Exception as e:
                    logger.error(f"Error inserting IVF telemetry data for tank {tank_id}: {e}", exc_info=True)
                    print(f"  ✗ Error inserting IVF telemetry data: {e}", flush=True)
                
                logger.info(f"✓ Successfully processed IVF shipment monitoring for tank {tank_id}")
                print(f"✓ Successfully processed IVF shipment monitoring for tank {tank_id} at {ivf_data['timestamp']}", flush=True)
                ivf_processed = True
