...
# Setup logging with immediate output (before imports that might use logger)
logging.basicConfig(
    level=logging.INFO,
...
# Try to import app models and services at top level (may fail if app directory not available)
try:
    ...
    IVF_QUALITY_LOG_MODEL_AVAILABLE = True
except ImportError as e:
    CRITICAL_ALERT_SERVICE_AVAILABLE = False
    CriticalAlertService = None
    IVF_QUALITY_LOG_MODEL_AVAILABLE = False
    IVFQualityLog = None
    logger.warning(f"Could not import app models/services: {e}. Event listeners may not work.")
...
def find_tank_by_tive_device_id(device_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
    Find tank by tive_device_id (for IVF tank monitoring scenario).
    Queries directly from tanks table using device_id.
    Creates its own database session.
    
    Returns dict with: tank_id, tank_code, canister_id (first active canister in tank), or None if not found.
    """
    if not device_id:
        return None
    ...
    db = PublisherSessionLocal()
    try:
        # First try direct lookup from tanks table
        query = text("""
            SELECT 
                t.tank_id,
                t.tank_code,
                t.is_active
            FROM tanks t
            WHERE t.tive_device_id = :device_id
        ...
        if row:
            ...
            # Get first active canister in the tank (for alert service compatibility)
            canister_query = text("""
                SELECT canister_id, canister_number
                FROM canisters
            ...
            canister_result = db.execute(canister_query, {"tank_id": tank_id})
            canister_row = canister_result.fetchone()
            
            canister_id = canister_row[0] if canister_row else None
            canister_number = canister_row[1] if canister_row else None
            
            return {
                "tank_id": tank_id,
                "tank_code": tank_code,
                "canister_id": canister_id,  # First canister in tank (for alert service)
...
def insert_ivf_telemetry_data(tank_id: int, data: Dict[str, Any]) -> bool:
    """
    ...
    Args:
        tank_id: Tank ID for monitoring/tracking (e.g., tank with code "T1")
        data: Telemetry data dictionary
    """
    db = PublisherSessionLocal()
    try:
        # Get tank_code for logging
        tank_query = text("SELECT tank_code FROM tanks WHERE tank_id = :tank_id")
        tank_result = db.execute(tank_query, {"tank_id": tank_id})
        tank_row = tank_result.fetchone()
        tank_code = tank_row[0] if tank_row else f"Tank-{tank_id}"
        ...
        # Insert into ivf_telemetry_data table (tank-level monitoring)
        insert_query = text("""
        ...
        logger.info(f"✓ Inserted IVF telemetry data for tank {tank_code} (tank_id={tank_id}, id={ivf_telemetry_data_id})")
        print(f"  → Inserted into database table 'ivf_telemetry_data' (id={ivf_telemetry_data_id}, tank: {tank_code})", flush=True)
        
        # Insert into ivf_quality_log table
        try:
            insert_ivf_quality_log(db, ivf_telemetry_data_id, tank_id, data, created_at)
        except Exception as e:
        ...
        try:
            insert_ivf_geolocation(db, ivf_telemetry_data_id, tank_id, data, created_at)
        except Exception as e:
        ...
        # Note: Automatic alert creation and email sending via SQLAlchemy event listener has been disabled.
        # To trigger alerts manually, use the CriticalAlertService.check_and_create_alerts() method via API endpoint.
        
        logger.debug(f"Quality log entry saved for tank {tank_code} - alerts can be triggered manually via API if needed")
        print(f"  → Quality log entry saved for tank {tank_code}", flush=True)
        
        return True
                
    except Exception as e:
        db.rollback()
        logger.error(f"Error inserting IVF telemetry data for tank {tank_id}: {e}", exc_info=True)
...
def insert_ivf_quality_log(db, ivf_telemetry_data_id: int, tank_id: int, 
    ...
    Args:
        db: Database session
        ivf_telemetry_data_id: Telemetry data ID
    ...
    try:
        ...
        # Get threshold violations from data to determine which parameters violated
        threshold_violations = data.get("threshold_violations", {})
        
        # Extract violation flags for each parameter
        # IVF KPIs: temp_internal, temp_external, humidity, shock
        is_temp_internal_loss = threshold_violations.get("temp_internal", False)
        is_temp_external_loss = threshold_violations.get("temp_external", False)
        is_humidity_loss = threshold_violations.get("humidity", False)
        is_shock_loss = threshold_violations.get("shock", False)
        
        # Only insert if there are violations (red/yellow deviations)
        if not has_violations:
            logger.debug(f"Skipping ivf_quality_log insert - no violations detected (quality_status={quality_status}, quality_loss={quality_loss}, kpi_statuses={kpi_statuses})")
        ...
        # Use ORM model to insert so that SQLAlchemy event listeners fire automatically
        # This ensures alerts are created via the after_insert event listener
        if IVF_QUALITY_LOG_MODEL_AVAILABLE and IVFQualityLog:
            ...
            quality_log = IVFQualityLog(
                telemetry_data_id=ivf_telemetry_data_id,
                tank_id=tank_id,  # Tank-level monitoring
                device_id=data.get("device_id"),
            ...
            db.add(quality_log)
            db.flush()  # Flush to get the ID and trigger the event listener
            quality_log_id = quality_log.id
            
            logger.info(f"✓ Inserted into ivf_quality_log table using ORM (id={quality_log_id}, telemetry_data_id={ivf_telemetry_data_id}, tank_id={tank_id}, quality_loss={quality_loss})")
            print(f"  → Inserted into ivf_quality_log table (id={quality_log_id}, quality_loss={quality_loss}) - event listener will trigger alerts automatically", flush=True)
        else:
            # Fallback to raw SQL if ORM model not available (shouldn't happen in normal operation)
            logger.warning("IVFQualityLog ORM model not available, using raw SQL (event listeners will NOT fire)")
            insert_query = text("""
                INSERT INTO ivf_quality_log 
                (telemetry_data_id, tank_id, device_id, temperature, humidity, agitation, 
                 quality_loss, is_temp_loss, is_humidity_loss, is_agitation_loss, is_light_loss, reading_timestamp, created_at)
                VALUES (:telemetry_data_id, :tank_id, :device_id, :temperature, :humidity, :agitation,
                        :quality_loss, :is_temp_loss, :is_humidity_loss, :is_agitation_loss, :is_light_loss, :reading_timestamp, :created_at)
            """)
            db.execute(insert_query, {
                "telemetry_data_id": ivf_telemetry_data_id,
                "tank_id": tank_id,
                "device_id": data.get("device_id"),
                "temperature": data.get("temp_internal"),
                "humidity": data.get("humidity"),
                "agitation": data.get("shock"),
                "quality_loss": quality_loss,
                "is_temp_loss": is_temp_internal_loss or is_temp_external_loss,
                "is_humidity_loss": is_humidity_loss,
                "is_agitation_loss": is_shock_loss,
                "is_light_loss": False,
                "reading_timestamp": created_at,
                "created_at": created_at
            })
            logger.info(f"✓ Inserted into ivf_quality_log table using raw SQL (telemetry_data_id={ivf_telemetry_data_id}, tank_id={tank_id}, quality_loss={quality_loss})")
...
def insert_ivf_geolocation(db, ivf_telemetry_data_id: int, tank_id: int, 
                          data: Dict[str, Any], created_at: datetime) -> bool:
    """
    ...
    try:
        ...
        logger.info(f"✓ Inserted into ivf_geolocation table (telemetry_data_id={ivf_telemetry_data_id}, tank_id={tank_id})")
        print(f"  → Inserted into ivf_geolocation table (telemetry_data_id={ivf_telemetry_data_id}, tank_id={tank_id})", flush=True)
        return True
...
def get_ivf_violation_frequency(tank_id: int, parameter_name: str, window_minutes: int = 60) -> Dict[str, Any]:
    """
    ...
    Args:
        tank_id: Tank ID for tank-level monitoring
        parameter_name: Parameter name (temp_internal, temp_external, humidity, shock)
    ...
    try:
        ...
        violation_flag_map = {
            ...
            "temperature": "is_temp_loss",  # Alias for backward compatibility
            "humidity": "is_humidity_loss",
            "shock": "is_agitation_loss",  # Shock uses is_agitation_loss flag
            "agitation": "is_agitation_loss",  # Alias for backward compatibility
            "light": "is_light_loss"
        }
        ...
        # Query violations in the time window (tank-level monitoring)
        query = text(f"""
...
def get_ivf_violation_duration(tank_id: int, parameter_name: str, current_violation: bool) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        # This finds the start of the current violation sequence (tank-level monitoring)
        query = text(f"""
...
def transform_webhook_to_ivf_quality_data(webhook_response: Dict[str, Any], ivf_shipment_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Get tank_id for tank-level monitoring (tanks table has tive_device_id)
    tank_id = ivf_shipment_info.get("tank_id")
    canister_id = ivf_shipment_info.get("canister_id")  # Only used for alert service compatibility
    
    # If tank_id not provided, try to get it from canister_id (shouldn't happen in normal flow)
    if not tank_id and canister_id:
        db_temp = PublisherSessionLocal()
        try:
            tank_query = text("SELECT tank_id FROM canisters WHERE canister_id = :canister_id")
            tank_result = db_temp.execute(tank_query, {"canister_id": canister_id})
            tank_row = tank_result.fetchone()
            if tank_row:
                tank_id = tank_row[0]
        except:
    ...
    for param_name in ivf_kpi_names:
        ...
        # Get frequency (count of violations in time window) - use tank_id for tank-level monitoring
        if tank_id:
            frequency_result = get_ivf_violation_frequency(tank_id, param_name, window_minutes=60)
        else:
            # Fallback: if no tank_id, return empty result (shouldn't happen in normal flow)
            logger.warning(f"No tank_id available for frequency check, skipping")
            frequency_result = {"count": 0, "frequency_status": "Green", "window_minutes": 60}
        frequency_results[param_name] = frequency_result
        
        # Get duration (how long current violation has persisted) - use tank_id for tank-level monitoring
        current_violation = magnitude_result.get("is_violation", False)
        if tank_id:
            duration_result = get_ivf_violation_duration(tank_id, param_name, current_violation)
        else:
            # Fallback: if no tank_id, return empty result (shouldn't happen in normal flow)
            logger.warning(f"No tank_id available for duration check, skipping")
            duration_result = {"duration_minutes": 0.0, "duration_status": "Green", "violation_start": None}
...
def process_webhook_data(webhook_response: Dict[str, Any]):
    """Process webhook data and publish to Redis for both CGT (patients) and IVF (tanks)"""
    try:
        ...
        try:
            ...
            # Check 1: Tank monitoring - fetch tive_device_id directly from tanks table (tank-level monitoring)
            # Tank-level monitoring only: tanks table has tive_device_id
            tank_info = None
            if device_id:
                # Find tank directly from tanks table using tive_device_id
                tank_info = find_tank_by_tive_device_id(device_id)
                if tank_info:
                    logger.info(f"✓ Found IVF tank from tanks table: tank_id={tank_info.get('tank_id')}, tank_code={tank_info.get('tank_code')}, device_id={device_id}")
                    print(f"✓ Found IVF tank from tanks table: tank_id={tank_info.get('tank_id')}, device_id={device_id}", flush=True)
                else:
                    logger.warning(f"No tank found with tive_device_id={device_id} in tanks table")
                    print(f"✗ No tank found with device_id={device_id} in tanks table", flush=True)
        ...
        # ========================================================================
        # STEP 2: Process IVF tank monitoring (if found)
        # Tank-level monitoring: fetches tive_device_id directly from tanks table
        # ========================================================================
        if tank_info:
            try:
                # Ensure we have tank_code (fetch if missing)
                if not tank_info.get("tank_code") and tank_info.get("tank_id"):
                    db_fallback = PublisherSessionLocal()
                    try:
                        tank_query = text("SELECT tank_code FROM tanks WHERE tank_id = :tank_id")
                        tank_result = db_fallback.execute(tank_query, {"tank_id": tank_info.get("tank_id")})
                        tank_row = tank_result.fetchone()
                        if tank_row:
                            tank_info["tank_code"] = tank_row[0]
                    except:
                        pass
                    finally:
                        db_fallback.close()
                
                logger.info("Processing IVF tank monitoring webhook...")
                print(f"Processing IVF tank monitoring webhook...", flush=True)
                
                tank_id = tank_info.get("tank_id")
                tank_code = tank_info.get("tank_code") or f"Tank-{tank_id}"
                canister_id = tank_info.get("canister_id")  # Keep for backward compatibility if needed
                
                # Convert to same format as ivf_shipment_info for processing
                tank_ivf_info = {
                    "shipment_id": None,
                    "iot_shipment_id": None,
                    "tank_id": tank_id,  # Tank-level monitoring (primary)
                    "canister_id": canister_id,  # Keep for backward compatibility if needed
                    "device_id": device_id,
                    "cryolock_id": None,
                    "source_branch_id": None,
                    "destination_branch_id": None,
                    "canister_number": tank_info.get("canister_number")
                }
                
                # Transform webhook data for IVF
                ivf_data = transform_webhook_to_ivf_quality_data(webhook_response, tank_ivf_info)
                
                # Publish to IVF Redis channel (using tank_id for tracking)
                try:
                    r.publish('ivf_quality_channel', json.dumps(ivf_data))
                    logger.info(f"Published to Redis channel 'ivf_quality_channel' for tank {tank_code} (tank_id={tank_id}, tank monitoring)")
                    print(f"  → Published to Redis channel 'ivf_quality_channel' (tank: {tank_code})", flush=True)
                except Exception as e:
                    logger.error(f"Error publishing to IVF Redis channel for tank {tank_code}: {e}")
                    print(f"  ✗ Error publishing to IVF Redis channel: {e}", flush=True)
                
                # Store in tank-specific history list (changed from canister to tank)
                try:
                    history_key = f'ivf_quality_history:tank_{tank_id}'
                    r.lpush(history_key, json.dumps(ivf_data))
                    r.ltrim(history_key, 0, 9)  # Keep last 10 messages
                    logger.info(f"Stored in Redis list '{history_key}' for tank {tank_code} (tank monitoring)")
                    print(f"  → Stored in Redis list '{history_key}' (tank: {tank_code})", flush=True)
                except Exception as e:
                    logger.error(f"Error storing in IVF Redis list for tank {tank_code}: {e}")
                    print(f"  ✗ Error storing in IVF Redis list: {e}", flush=True)
                
                # Store tank ID in set (changed from canister to tank)
                try:
                    r.sadd('ivf_tanks', str(tank_id))
                    logger.debug(f"Added tank {tank_code} (tank_id={tank_id}) to Redis set 'ivf_tanks'")
                except Exception as e:
                    logger.error(f"Error adding tank {tank_code} to Redis set: {e}")
                
                # Insert into IVF database tables (tank-level monitoring)
                try:
                    insert_ivf_telemetry_data(tank_id, ivf_data)
                    logger.info(f"✓ Inserted IVF telemetry data for tank {tank_code} (tank_id={tank_id}, tank monitoring)")
                    print(f"✓ Inserted IVF telemetry data for tank {tank_code} (tank monitoring)", flush=True)
                except Exception as e:
                    logger.error(f"Error inserting IVF telemetry data for tank {tank_code}: {e}", exc_info=True)
                    print(f"  ✗ Error inserting IVF telemetry data: {e}", flush=True)
                
                logger.info(f"✓ Successfully processed IVF tank monitoring for tank {tank_code} (tank_id={tank_id})")
                print(f"✓ Successfully processed IVF tank monitoring for tank {tank_code} at {ivf_data['timestamp']}", flush=True)
                ivf_processed = True
                
            except Exception as e:
                logger.error(f"Error processing IVF tank monitoring: {e}", exc_info=True)
                print(f"✗ Error processing IVF tank monitoring: {e}", flush=True)
                import traceback
                traceback.print_exc()
        
        # ========================================================================
        # STEP 3: Process IVF shipment monitoring (if found - runs even if tank was processed)
        # Changed to use tank-level monitoring
        # ========================================================================
        if ivf_shipment_info:
            try:
                ...
                # Get tank information from shipment's canister
                canister_id = ivf_shipment_info.get("canister_id")
                db_shipment = PublisherSessionLocal()
                try:
                    # Get tank_id from canister
                    tank_query = text("SELECT tank_id, tank_code FROM tanks t INNER JOIN canisters c ON t.tank_id = c.tank_id WHERE c.canister_id = :canister_id")
                    tank_result = db_shipment.execute(tank_query, {"canister_id": canister_id})
                    tank_row = tank_result.fetchone()
                    if tank_row:
                        tank_id = tank_row[0]
                        tank_code = tank_row[1] or f"Tank-{tank_id}"
                    else:
                        # Fallback: use canister_id as tank_id (shouldn't happen, but handle gracefully)
                        tank_id = None
                        tank_code = None
                        logger.warning(f"Could not find tank for canister_id {canister_id} in shipment monitoring")
                except Exception as e:
                    logger.error(f"Error getting tank info for shipment: {e}")
                    tank_id = None
                    tank_code = None
                finally:
                    db_shipment.close()
                
                # Add tank_id to ivf_shipment_info for transform function
                ivf_shipment_info_with_tank = ivf_shipment_info.copy()
                if tank_id:
                    ivf_shipment_info_with_tank["tank_id"] = tank_id
                
                # Transform webhook data for IVF
                ivf_data = transform_webhook_to_ivf_quality_data(webhook_response, ivf_shipment_info_with_tank)
                
                # Publish to IVF Redis channel (using tank_id for tracking)
                try:
                    r.publish('ivf_quality_channel', json.dumps(ivf_data))
                    logger.info(f"Published to Redis channel 'ivf_quality_channel' for tank {tank_code} (tank_id={tank_id}, shipment monitoring)")
                    print(f"  → Published to Redis channel 'ivf_quality_channel' (tank: {tank_code}, shipment monitoring)", flush=True)
                except Exception as e:
                    logger.error(f"Error publishing to IVF Redis channel for tank {tank_code}: {e}")
                    print(f"  ✗ Error publishing to IVF Redis channel: {e}", flush=True)
                ...
                if tank_id:
                    try:
                        ...
                        r.lpush(history_key, json.dumps(ivf_data))
                        r.ltrim(history_key, 0, 9)  # Keep last 10 messages
                        logger.info(f"Stored in Redis list '{history_key}' for tank {tank_code} (shipment monitoring)")
                        print(f"  → Stored in Redis list '{history_key}' (tank: {tank_code})", flush=True)
                    except Exception as e:
                        logger.error(f"Error storing in IVF Redis list for tank {tank_code}: {e}")
                        print(f"  ✗ Error storing in IVF Redis list: {e}", flush=True)
                ...
                if tank_id:
                    try:
                        insert_ivf_telemetry_data(tank_id, ivf_data)
                        logger.info(f"✓ Inserted IVF telemetry data for tank {tank_code} (tank_id={tank_id}, shipment monitoring)")
                        print(f"✓ Inserted IVF telemetry data for tank {tank_code} (shipment monitoring)", flush=True)
                    except Exception as e:
                        logger.error(f"Error inserting IVF telemetry data for tank {tank_code}: {e}", exc_info=True)
                        print(f"  ✗ Error inserting IVF telemetry data: {e}", flush=True)
                else:
                    logger.warning(f"Skipping database insert for shipment - no tank_id found")
                
                logger.info(f"✓ Successfully processed IVF shipment monitoring for tank {tank_code} (tank_id={tank_id})")
                print(f"✓ Successfully processed IVF shipment monitoring for tank {tank_code} at {ivf_data['timestamp']}", flush=True)
                ivf_processed = True
